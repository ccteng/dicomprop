...
void* g_fragment = NULL;
U32 g_fragmentLength = 0;
				 ...
				 unsigned long opt, DCM_OBJECT ** callerObject)
{
	...
	cond = readFile1("", buf, -1, length, 0, 0, opt, NULL, callerObject, NULL, NULL,
					 NULL, NULL, NULL);

    if ((cond == DCM_NORMAL) && g_fragment)
    {
        DCM_AddFragment(callerObject, g_fragment, g_fragmentLength);
    }
...
readGroupElement(const char *name, unsigned char **ptr, int fd, U32 * size,
				 ...
				 PRIVATE_OBJECT ** object, U32 * scannedLength,
				 DCM_ELEMENT * e, CTNBOOLEAN skipPixelData)
{
	...
	if ((e->tag == DCM_PXLPIXELDATA) && skipPixelData)
	{
		// ccteng
		// we're only interested in the header so skip the pixel data
		// don't scan them
		if (scannedLength)
		{
			*scannedLength += *size;
	...
	* sequenceObject;
	...
	while (!sequenceDone)
	{
		...
		flag = readGroupElement(name, ptr, fd, &localLength, fileOffset, knownLength,
								byteOrder, explicitVR, acceptVRMismatch, object, &sequenceLength, &tagE, FALSE);
					 ...
					 PRV_ELEMENT_ITEM ** elementItem)
{
	...
	while (!sequenceDone)
	{
		...
		flag = readGroupElement(name, ptr, fd, &localLength, fileOffset,
								...
								object, &sequenceLength, &tagE, FALSE);
		if (flag == DCM_STREAMCOMPLETE)
			break;
		else if (flag != DCM_NORMAL)
			return flag;
		...
		if (tagE.tag == DCM_DLMITEM)
		{
			...
			if (tagE.length != 0)
			{
                ...
                {
                    // add fragment
                    // DCM_AddFragment((DCM_OBJECT**)object, *ptr, tagE.length);
                    g_fragment = *ptr;
                    g_fragmentLength = tagE.length;
...
/*	if (fileFlag) */
	...
	else
	{
		...
		{
            // CCTENG
			if (pixelFlag && ((*elementItem)->element.length == DCM_UNSPECIFIEDLENGTH))
			{
//				(*elementItem)->element.length = (*elementItem)->originalDataLength = (*elementItem)->paddedDataLength = *size;
                ...
                s1 = *size;
                cond = scanCompressedPixels("", ptr, -1,
                                     &s1,	/* size */
                                     NULL,	/* fileOffset */
                                     0, 0,
                                     byteOrder, explicitVR,
		  ...
		  CONDITION(*sk) (void *ctx, int offset, int flag))
{
	...
	* elementItem = NULL;
	...
	CTNBOOLEAN skipPixelData = FALSE;
	...
	if ((opt & DCM_SKIPPIXELDATA) == DCM_SKIPPIXELDATA)
	{
		skipPixelData = TRUE;
	}
	...
	while (!done)
	{
		...
		if (e.representation == DCM_SQ)
		{
			...
			flag = readSequence(name, &ptr, fd, &sequenceLength,
								fileOffset, recursionLevel, opt & ~DCM_SKIPPIXELDATA,
...
DCM_AddFragment(DCM_OBJECT** callerObject, void* fragment, U32 fragmentLength)
{
    ...
    else
    {
        // CCTENG
        // 
        // What do we do with the old d value???
        
		elementItem->element.d.fragments = LST_Create();
    }
